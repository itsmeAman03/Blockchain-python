...
def test_transaction_exceeds_balance():
    ...
    with pytest.raises(Exception,match='Amount exceeds balance'):
        Transaction(Wallet(),'recipient',99999)

def test_transaction_update_exceeds_balance():
    sender_wallet=Wallet()
    transaction=Transaction(sender_wallet,'recipient',96)

    with pytest.raises(Exception,match='Amount exceeds balance'):
        Transaction(Wallet(),'new_recipient',99999)
...
def test_transaction_update():
    ...
    assert transaction.output[first_recipient] == to_first_again + first_amount

    assert transaction.output[sender_wallet.address] == sender_wallet.balance - (first_amount + next_amount+ to_first_again)
